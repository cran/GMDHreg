...
#'
#' @description Build a regression model performing GMDH Twice-Multilayered Combinatorial (TMC) with PRESS criteria.
#'
#' @keywords internal
#'
gmdh.combi.twice_1 <- function(X, y, G) {
  ...
  while(fin.1 >= fin.2) {
    ...
    results <- apply(combs, 1, function(x){fun.svd_1(y = y, x = regressors[, x, drop = FALSE])})
    ...
    results.2 <- apply(combs, 1, function(x){fun.svd_1(y = y, x = regressors[, x, drop = FALSE])})
